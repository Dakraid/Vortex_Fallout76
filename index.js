...
function updateArchiveList(profileId, api) {
    ...
    return fs.readdirAsync(dataFolder)
    .then((dataFiles) => {
        // Get a list of all BA2s in the data folder.
        const dataFolderBA2s = dataFiles.filter(f => path.extname(f) === ARCHIVE_EXT);
        // Read our INI
        return parser.read(fallout76Custom)
        .then((ini) => {
            // Get the current INI list and convert it to an array.
            const iniList = ini.data.Archive.sResourceArchive2List;
            const iniArray = iniList.length ? iniList.split(',').map(b => b.trim()) : [];
            // Filter out any missing ba2s
            let newArchives = iniArray.filter(b => dataFolderBA2s.includes(b));
            // Add any new BA2s that don't exist. 
            newArchives = newArchives.concat(activeBA2s.filter(b => dataFolderBA2s.includes(b) && !newArchives.includes(b))).join(', ');
            // If the two lists don't match, update them. 
            if (newArchives !== iniList) {
                ini.data.Archive.sResourceArchive2List = newArchives;
    ...
    .catch(err => log('error', 'Error reading data folder for Fallout 76', err));
}
...
function main(context) {
    ...
    context.once(() => {
        ...
        // When deploying, build the list of BA2s
        context.api.events.on('did-deploy', (profileId) => updateArchiveList(profileId, context.api));
    });
